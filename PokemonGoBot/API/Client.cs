...
namespace PokemonGoBot.API
{
    public class Client
    {
       ...
       //private ISettings _settings;
        ...
        public Client(ISettings settings, int id)
        {
        ...
        public async Task<CatchPokemonResponse> CatchPokemon(int id, ulong encounterId, string spawnPointGuid, double pokemonLat,
            double pokemonLng, MiscEnums.Item pokeball, int? pokemonCP)
        {
            var customRequest = new Request.Types.CatchPokemonRequest
            {
                EncounterId = encounterId,
                Pokeball = (int)GetBestBall(id, pokemonCP).Result,
        ...
        public async Task DoGoogleLogin(ISettings client, int id)
        {
            var _settings = Main.clientData[id];
            _authType = AuthType.Google;
            GoogleLogin.TokenResponseModel tokenResponse = null;

            if (string.IsNullOrEmpty(_settings.GoogleRefreshToken) && string.IsNullOrEmpty(client.AccessToken))
            {
                var deviceCode = await googleLogin.GetDeviceCode(id);
                tokenResponse = await googleLogin.GetAccessToken(deviceCode);
                _accessToken = tokenResponse.id_token;
                _log.Log_(id, Color.White, $"Put RefreshToken in settings for direct login: {tokenResponse.refresh_token}");
                _settings.GoogleRefreshToken = tokenResponse.refresh_token;
                client.AccessToken = tokenResponse.refresh_token;
            }
            else
            {
                if (!string.IsNullOrEmpty(_settings.GoogleRefreshToken))
                    ...
                    tokenResponse = await googleLogin.GetAccessToken(client.AccessToken);
                _accessToken = tokenResponse.id_token;
            }
        }
        ...
        private async Task<MiscEnums.Item> GetBestBall(int id, int? pokemonCP)
        {
