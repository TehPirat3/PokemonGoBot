...
namespace PokemonGoBot.API.Login
{
    public class GoogleLogin
    {
        ...
        public async Task<TokenResponseModel> GetAccessToken(DeviceCodeModel deviceCode)
        {
        ...
        public async Task<TokenResponseModel> GetAccessToken(string refreshToken)
        {
        ...
        public async Task<DeviceCodeModel> GetDeviceCode(int id)
        {
            ...
            try
            {
                _log.Log_(id, Color.Green, "Google Device Code copied to clipboard");
                //_log.Log_(id, Color.Green, $"Goto: http://www.google.com/device & enter {deviceCode.user_code}");
                Thread.Sleep(2000);
        ...
        private async Task<TokenResponseModel> PollSubmittedToken(string deviceCode)
        {
            return await HttpClientHelper.PostFormEncodedAsync<TokenResponseModel>(OauthTokenEndpoint,
                new KeyValuePair<string, string>("client_id", ClientId),
                new KeyValuePair<string, string>("client_secret", ClientSecret),
